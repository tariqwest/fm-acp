#!/usr/bin/env node
/**
 * Coupled project release: GitHub release asset + Homebrew tap formulas.
 *
 * Default behavior (both steps run unless opted out):
 *   1. optional tests/typecheck
 *   2. build prebuilt tarball
 *   3. git tag + GitHub release upload
 *   4. generate fm-acp + cua-driver formulas and push tariqwest/homebrew-tap
 *
 * Usage:
 *   bun scripts/release.mjs [options]
 *   bun run release -- [options]
 *
 * Options:
 *   --dry-run              Print actions; still builds local tarball/formula copies
 *   --skip-tests           Skip bun test + typecheck
 *   --skip-github          Skip tag/GitHub release (Homebrew-only path)
 *   --skip-homebrew        Skip formula generation + tap push (GitHub-only path)
 *   --homebrew-only        Alias for --skip-github
 *   --github-only          Alias for --skip-homebrew
 *   --skip-release         Deprecated alias for --skip-github
 *   --skip-tap-push        Write formulas locally but do not commit/push the tap
 *   --tap-path PATH        Override homebrew-tap checkout
 *
 * Env:
...
// Coupled by default: GitHub release + Homebrew tap.
const skipGithub =
  args.has("--skip-github") ||
  args.has("--homebrew-only") ||
  args.has("--skip-release"); // deprecated alias
const skipHomebrew =
  args.has("--skip-homebrew") ||
  args.has("--github-only");
const skipTests = args.has("--skip-tests") || dryRun;
const skipTapPush = args.has("--skip-tap-push") || dryRun || skipHomebrew;
...
if (skipGithub && skipHomebrew) {
  console.error("[release] nothing to do: both --skip-github and --skip-homebrew set");
  process.exit(2);
}
...
function runChecks() {
  if (skipTests) {
    console.error("[release] skip tests (--skip-tests or --dry-run)");
    return;
  }
  console.error("[release] running tests + typecheck…");
  const hasBun = spawnSync("bun", ["--version"], { encoding: "utf8" }).status === 0;
  if (hasBun) {
    run("bun", ["test"], { stdio: "inherit" });
    run("bun", ["run", "typecheck"], { stdio: "inherit" });
  } else {
    run("pnpm", ["run", "test:node"], { stdio: "inherit" });
    run("pnpm", ["run", "typecheck"], { stdio: "inherit" });
  }
}

function ensureCleanGitForRelease() {
  if (dryRun || skipGithub) return;
  const status = run("git", ["status", "--porcelain"]);
  if (status) {
    throw new Error(
      `git working tree is dirty; commit/stash before release:\n${status}`,
    );
  }
}
...
function main() {
  console.error(
    `[release] version=${version} tag=${tag} dryRun=${dryRun} github=${!skipGithub} homebrew=${!skipHomebrew}`,
  );

  runChecks();
  ensureCleanGitForRelease();

  // Always build the prebuilt artifact when either channel needs it.
  const prebuilt = buildPrebuiltTarball();
  console.error(`[release] tarball=${prebuilt.tarball}`);
  console.error(`[release] sha256=${prebuilt.digest}`);

  if (!skipGithub) {
    // Ensure main is on remote before/with tag when possible.
    if (!dryRun) {
      try {
        run("git", ["push", "origin", "HEAD"], { stdio: "inherit" });
      } catch (err) {
        console.error(`[release] warn: push HEAD failed: ${err.message}`);
      }
    }
    ensureRelease({ tarball: prebuilt.tarball });
  } else {
    console.error("[release] skip GitHub release (--skip-github / --homebrew-only)");
  }

  if (!skipHomebrew) {
    const tapPath = detectTapPath();
    console.error(`[release] tap=${tapPath}`);
    const cua = fetchLatestCuaDriver();
    console.error(`[release] cua-driver ${cua.version} sha256=${cua.sha256}`);
    ...
    console.error("[release] skip Homebrew (--skip-homebrew / --github-only)");
  }

  console.error("[release] done");
  if (!skipGithub) {
    console.error(`  release: https://github.com/${ownerRepo}/releases/tag/${tag}`);
  }
  if (!skipHomebrew) {
    console.error("  brew update && brew tap tariqwest/tap && brew install fm-acp");
  }
}
